...
Before doing anything, orient yourself with this sequence. It routes each request to the right depth and recovers work from past sessions instead of starting cold:

1. **Load what exists.** Look for `.specs/`. If it's there, read `PROJECT.md` and `STATE.md` (add `ROADMAP.md` when planning features); codebase docs load on demand (see Context Loading Strategy). This recovers prior decisions, blockers, and in-flight work so you don't contradict or repeat them.
2. **Assess scope.** Classify the request as Small / Medium / Large / Complex with the Auto-Sizing table below. Scope — not a fixed pipeline — decides which phases you actually run.
3. **Route to a phase.** Match the request against the Commands table (below) and open only that one reference. "Let's plan feature X" starts at Specify; "implement T3" jumps straight to Execute; "quick fix Y" takes the Quick-mode express lane.
4. **Confirm depth when unsure, then go.** If scope is ambiguous, say your read out loud — "This looks Medium: spec + implement, skipping formal design and tasks. Sound right?" — and let the user correct before you invest. When it's clearly bounded, just proceed.

No `.specs/` yet and the user is starting something new? That's project initialization (or brownfield mapping for an existing codebase) — see Workflow.
