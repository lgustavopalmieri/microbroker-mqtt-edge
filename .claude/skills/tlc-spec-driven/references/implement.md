...
Read [coding-principles.md](coding-principles.md), then state three things up front. This isn't ceremony — saying them out loud is what keeps the change surgical and catches a wrong assumption before it turns into wrong code:
...
If there's no `tasks.md` for this feature, list the atomic steps before writing any code. Jumping straight to the keyboard is how an agent loses focus and ends up doing five things at once — the list is your guardrail against that.
...
**This is the one place to stay rigid.** Most of this skill trusts your judgment; test integrity does not. The moment "make it pass" beats "keep it honest," the gate stops meaning anything and every later verification is built on a lie. So:

- Do not modify tests written in Step 4 — the tests are the spec, and implementation conforms to them.
- Do not weaken assertions to pass more easily.
- Do not delete or skip test cases.
- Do not use the test framework's skip/disable/pending mechanism to bypass failing tests.
- Write the minimum code to pass — save structural improvements for a refactor task.
...
Run the gate check command from the task definition — on every task, not just the ones that feel risky. The gate is what makes "done" objective: the test runner decides whether the code is correct, not your own read of it.

1. Look up the command for the task's Gate level (quick/full/build) in TESTING.md's Gate Check Commands section, then run it.
2. Non-zero exit code → fix the failure, re-run, and don't move on until it's green.
