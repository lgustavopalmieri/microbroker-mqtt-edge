...
If TESTING.md does not exist (greenfield project), ask the user what test types and commands the
project will use — then **write the answers to `.specs/codebase/TESTING.md`** before creating tasks
(at minimum the Test Coverage Matrix and Gate Check Commands sections — see the TESTING.md template
in [brownfield-mapping.md](brownfield-mapping.md)). Persisting it matters: Execute and Validate read
TESTING.md every session to find the gate commands, so a verbal-only agreement is lost the moment the
session ends and the gates silently degrade. Create the file once, here.
...
Before showing tasks to the user, run all three pre-approval checks below. They exist because the three artifacts here — task bodies, the execution diagram, and the test matrix — drift out of sync easily, and a user who approves inconsistent tasks bakes that inconsistency straight into the build. If a check fails, restructure and re-run until they all pass.
...
**Output both tables with the tasks** so the user can see the validation results. Any ❌ means restructuring before presenting — don't hand the user failing tasks and ask them to approve.

### 6. Ask About MCPs and Skills

Before execution, ask the user which tools fit each task. Naming the right MCP or skill up front steers each task toward the correct approach instead of a generic one:
...
**Where**: `path/to/x_interface`
**Depends on**: None
**Reuses**: `path/to/base_interface`
**Requirement**: [FEAT]-01
...
- [ ] Types/contracts exported correctly
- [ ] Compiles / type-checks with no errors
...
**Where**: `path/to/y_service`
**Depends on**: T1
**Reuses**: `path/to/base_service` patterns
...
**What**: [Exact deliverable]
**Where**: `path/to/z_component`
**Depends on**: T1
**Reuses**: `path/to/base_component`
...
**What**: [Exact deliverable]
**Where**: `path/to/y_service` (modify)
**Depends on**: T2, T3
