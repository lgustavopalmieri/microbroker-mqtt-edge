...
- **Test Coverage Matrix:** Sample 5-10 existing `_test.go` files to identify which layers are tested and how. Go co-locates tests (`foo_test.go` sits next to `foo.go`), so the file's package and path reveal the layer. Extract run commands from the `Makefile`/`Taskfile.yml`, `go test` invocations, and CI config (`.github/workflows`). Mark layers with no existing tests as "none" with a note in CONCERNS.md.
- **Parallelism Assessment:** NOT parallel-safe signals: tests that omit `t.Parallel()`; package-level/global state mutated across tests; a shared DB connection (same DSN from config/env); table-level cleanup in setup or teardown (`db.Exec("DELETE FROM ...")`, `TRUNCATE`); `t.Setenv` (which makes `t.Parallel()` illegal in that test). Parallel-safe signals: tests that call `t.Parallel()`; per-test DB creation (in-memory SQLite `:memory:`, a unique temp DB per test, or testcontainers-go); `t.TempDir()` for filesystem isolation; data namespacing (all rows keyed by a unique test ID); no shared mutable globals; dependencies mocked via gomock (`go.uber.org/mock`).
