...
Run the Build-level gate check from TESTING.md — this is the objective proof the feature works, so it runs before any quality review.

For greenfield projects, TESTING.md was created during the Tasks phase, so its Gate Check Commands are already persisted — use the Build-level command. If for some reason it's still missing, agree the command with the user and write it to `.specs/codebase/TESTING.md` now so the next session inherits it.

1. Run: `[build gate command from TESTING.md, or the command agreed during planning]`
2. Non-zero exit code → stop here and fix it before moving to the Code Quality Check.
3. Record results:
   - Total test count: [N]
   - Passed: [N]
   - Failed: [list]
...
The gate check proves the code *works*; this check proves it's the *right* code — minimal, surgical, in-pattern. For each changed file, verify against [coding-principles.md](coding-principles.md):
...
- **Quality check isn't skippable** — green tests don't prove the code is minimal or in-pattern
