...
func setupTestServer(t *testing.T) (*Server, chan Message, context.CancelFunc) {
	t.Helper()
	topics, err := domain.NewTopicRegistry([]string{"machine/status", "machine/alarm"})
	require.NoError(t, err)

	msgChan := make(chan Message, 100)
	connMgr := NewConnectionManager(5)
	auth := NewEnvAuthenticator("admin", "secret")
	ctx, cancel := context.WithCancel(context.Background())

	server := NewServer("127.0.0.1:0", connMgr, auth, topics, msgChan, "UTC", NopLogger{})
...
func TestHandler_PublishQoS0(t *testing.T) {
	...
	// Publish QoS 0
	conn.Write(buildPublishPacket("machine/status", []byte(`{"temp":42}`), 0, 0))
...
func TestHandler_PublishQoS1(t *testing.T) {
	...
	// Publish QoS 1
	conn.Write(buildPublishPacket("machine/alarm", []byte(`{"alarm":true}`), 1, 42))
...
func TestHandler_PublishDisallowedTopic(t *testing.T) {
	...
	conn.Write(buildPublishPacket("not/allowed", []byte("data"), 0, 0))
